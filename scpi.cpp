...
cNode* cNodeSCPI::TestNode(cCmdInterpreter* ci,char** inp)	{
    ...
    *inp=tinp; // hinter dem schlüsselwort gehts weiter
    ...
    default		: // es ist ein command oder eine query mit parameter
	 ...
	 if (c == '?') { // ?
	     m_nNodeStat |= (nNodeDef & isQuerywPar); // es ist eine query mit parameter wenns eine sein darf
	     return (NULL);   
	 }
	 
	 m_nNodeStat |= (nNodeDef & isCommand); // gesetzt wenn es ein kommando sein darf
